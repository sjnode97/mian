...
import { singlePowerDayMonthYear } from '@/api/index'
  ...
  methods: {
    getTimeData() {
      if (this.cockpit) {
        ...
        this.getPowerData(this.station)
      }
      // 15分钟请求一次
      this.time = setTimeout(this.getTimeData, 1000 * 60 * 15)
    },
    async getData() {
    ...
    async getPowerData(powerStationId) {
      await singlePowerDayMonthYear({ powerStationId }).then(({ data }) => {
        this.barData = data
      })
    }
  }
